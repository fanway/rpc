...
static void
calcprog_1(struct svc_req *rqstp, SVCXPRT *transp)
{
	union {
		double exp_1_arg;
		double pi_1_arg;
	...
	case PI:
		xdr_argument = (xdrproc_t) xdr_double;
		xdr_result = (xdrproc_t) xdr_double;
		local = (char *(*)(char *, struct svc_req *)) pi_1_svc;
		break;
...
main( int argc, char* argv[] )
{
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_DGRAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_UDP;
		if (!svc_register(transp, CALCPROG, CALCVERS, calcprog_1, proto)) {
			_msgout("unable to register (CALCPROG, CALCVERS, udp).");
			exit(1);
		}
	}
	...
	if ((_rpcfdtype == 0) || (_rpcfdtype == SOCK_STREAM)) {
		...
		if (!_rpcpmstart)
			proto = IPPROTO_TCP;
		if (!svc_register(transp, CALCPROG, CALCVERS, calcprog_1, proto)) {
			_msgout("unable to register (CALCPROG, CALCVERS, tcp).");
			exit(1);
		}
	}
